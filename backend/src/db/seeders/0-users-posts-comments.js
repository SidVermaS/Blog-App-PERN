...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    */
    ...
    const posts=postsData.map((item, index)=>({ ...item, id: uuid.v4(), user_id: users[index].id, likes_count: 0 }))
   ...
   const likes=posts.map((item, index)=>( { id: uuid.v4(), user_id: users[index].id, post_id: posts[index].id  } ))
