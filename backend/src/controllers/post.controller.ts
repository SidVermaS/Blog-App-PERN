...
const create = async (req: Request<any>, res: Response<any>) => {
  try {
    const id = uuidv4();
    req.body.file_type = Tables.posts;
    ...
    if (photo_url) {
      ...
      if (post) {
        return res
          .status(201)
          .json({ message: "Successfully created the post", post });
      } else {
    ...
    return res
      .status(500)
      .json({ message: "Failed to create the post", error });
  }
};
...
const fetch = async (req: Request<any>, res: Response<any>) => {
  try {
    ...
    if (post) {
      return res
        .status(200)
        .json({ message: "Successfully fetched the post", post });
    } else {
...
const fetchAll = async (req: Request<any>, res: Response<any>) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Successfully fetched the posts", posts });
  } catch (error) {
    return res
      .status(500)
      .json({ message: "Failed to fetch the posts", error });
  }
};
