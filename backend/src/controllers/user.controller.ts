...
import { usersC } from '../constants/tables.constant'

const signUp=async (req: Request<any>, res: Response<any>)=>    {
    try {
        let id, public_user_id=uuidv4(), name, email, phone, password
        req.body.file_type=usersC
        ...
        if(await User.findOne({ attributes: ['id'], 
            where: { 
                [ Sequelize.Op.or ]: [ { email, }, { phone } ] 
            } 
        })) {
            return res.status(409).json({ message: 'Email/Phone no is already registered' })
        }
        const user=await User.create({ name, email, phone, password, photo_url, public_user_id })
        if(user) {
            ({ id, name, email, phone, }=user)
            return res.status(201).json({ message: 'Successfully signed up', user: { id, name, email, phone, public_user_id } })
        } else {
...
const signIn=async (req: Request<any>, res: Response<any>)=> {
    try {
        ...
        const user:any=await User.findOne({ 
            attributes: ['id', 'public_user_id', 'phone', 'email', 'password', 'name', 'photo_url', ], 
            where: Number.isInteger(email_phone)?{ phone: email_phone }:{ email: email_phone, }
        })
        ...
        if(user) {
            if(await user.validPassword(password)) {
                ...
                return res.status(201).json({ message: 'Successfully signed in', user: { id, public_user_id, email, phone,name, photo_url }, token })
            } else {
                return res.status(400).json({ message: 'Email/phone is incorrect' })
            }           
        } else {
            return res.status(409).json({ message: 'Email/Phone no isn\'t registered' })
        }
    } catch (error) {
...
const fetchAll=async (req: Request<any>, res: Response<any>)=> {
    try {
        ...
        if(users) {
            return res.status(200).json({ message: 'Successfully fetched the users', users })
        } else {
            return res.status(500).json({ message: "Failed to fetch the users", });
        }
    } catch (error) {
        return res.status(500).json({ message: "Failed to fetch the users", error });
    }
}
